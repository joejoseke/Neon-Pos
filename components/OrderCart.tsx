    ...
    return (
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-lg flex flex-col h-full shadow-lg">
            <div className="p-4 border-b border-slate-700/50 flex justify-between items-center">
                <h2 className="text-lg sm:text-xl font-bold text-pink-400">Current Order</h2>
                <button
                    onClick={onClearCart}
            ...
            <div className="flex-grow p-4 overflow-y-auto">
                {cartItems.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                        {cartItems.map(item => (
                            <li key={item.id} className="flex items-center gap-2 sm:gap-4 bg-slate-700/30 p-2 rounded-md">
                                <div className="flex-grow">
                                    <p className="font-semibold text-slate-200">{item.name}</p>
                                    <p className="text-sm text-cyan-400">Ksh{item.price.toFixed(2)}</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className="bg-slate-600 w-6 h-6 rounded-full font-bold flex-shrink-0">-</button>
                                    <span className="w-8 text-center font-semibold">{item.quantity}</span>
                                    <button onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className="bg-slate-600 w-6 h-6 rounded-full font-bold flex-shrink-0">+</button>
                                </div>
                                <p className="font-bold w-16 sm:w-20 text-right">Ksh{(item.price * item.quantity).toFixed(2)}</p>
                            </li>
                        ))}
                    </ul>
                )}
            ...
            <div className="p-4 mt-auto border-t border-slate-700/50 bg-slate-800/80 rounded-b-lg">
                ...
                <button
                    onClick={onCharge}
                    disabled={cartItems.length === 0}
                    className="w-full mt-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-3 rounded-lg text-base sm:text-lg
